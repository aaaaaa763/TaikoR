        ...
        private void tコンボ表示_太鼓( int nCombo値, int nジャンプインデックス, int nPlayer )
		{
            ...
            for ( int i = 0; i < n桁数; i++ )
            {
                ...
                if ( n桁数 <= 1 )
                {
				    if (txTaiko_Combo0 != null)
				    {
						var yScalling = ComboScale[this.ctコンボ加算[nPlayer].n現在の値] / 1.5;
						txTaiko_Combo0.vc拡大縮小倍率.Y = (float)(TJAPlayer3.Skin.Game_Taiko_Combo_Scale[0] + yScalling);
                ...
                else
                {
                    ...
                    if( n桁数 == 3 )
                    {
                        ...
                        {
							var yScalling = ComboScale[this.ctコンボ加算[nPlayer].n現在の値] / 1.5;
							txTaiko_Combo1.vc拡大縮小倍率.Y = (float)(TJAPlayer3.Skin.Game_Taiko_Combo_Scale[1] + yScalling);
                            txTaiko_Combo1.vc拡大縮小倍率.X = TJAPlayer3.Skin.Game_Taiko_Combo_Scale[1];
                            txTaiko_Combo1.t2D拡大率考慮下中心基準描画( TJAPlayer3.app.Device, rightX - TJAPlayer3.Skin.Game_Taiko_Combo_Padding[1] * i, TJAPlayer3.Skin.Game_Taiko_Combo_Ex_Y[nPlayer], new Rectangle( n位の数[ i ] * TJAPlayer3.Skin.Game_Taiko_Combo_Size_Ex[0], 0, TJAPlayer3.Skin.Game_Taiko_Combo_Size_Ex[0], TJAPlayer3.Skin.Game_Taiko_Combo_Size_Ex[1]) );
                        }
